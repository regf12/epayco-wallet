<template>
  <card :title="$t('recharge')">
    <form @submit.prevent="recharge" @keydown="form.onKeydown($event)">
      ...
      <div class="form-group row">
        ...
        <div class="col-md-7">
          ...
          <has-error :form="form" field="document" />
			...
			<!-- Mount -->
			<div class="form-group row">
				<label class="col-md-3 col-form-label text-md-right">{{ $t('mount') }}</label>
				<div class="col-md-7">
					<input v-model="form.mount" :class="{ 'is-invalid': form.errors.has('mount') }" type="number" min="0" name="mount" required class="form-control">
...
export default {
  ...
  methods: {
    ...
    async recharge (){
			try {
        const response = await this.form.post('/operations/recharge')
